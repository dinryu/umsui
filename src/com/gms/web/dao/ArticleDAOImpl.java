...
public class ArticleDAOImpl implements ArticleDAO {
	...
	public List<ArticleBean> selectAll() {
		...
		try {
			...
			while(rs.next()) {
				...
				article.setSeq(Integer.parseInt(rs.getString(DB.BOARD_SEQ)));
				...
				article.setId(rs.getString(DB.MEMBER_REGDATE));
	...
	public String update(ArticleBean bean) {
		String rs ="";
		ArticleBean temp= selectBySeq(bean.getSeq());
		...
		try {
			...
			pstmt.setString(3, String.valueOf(bean.getSeq()));
			rs = String.valueOf(pstmt.executeUpdate());
		}
		catch(Exception e){
			e.printStackTrace();
